...
Here is the structural hierarchy shift:

```
Traditional:
  Epic > Feature > Story > Task
  Context: scattered across Jira fields, Confluence, human memory

BMAD:
  Product Brief > PRD > Architecture > Epics > Stories
  Context: pushed down into each story file
  File: docs/stories/1.1.add-item-to-cart.story.md
```
...
```
Phase 2: PM Agent produces PRD.md (requirements, personas, epic hierarchy)
Phase 3: Architect Agent produces architecture.md (Stripe API v2024-12-18,
         webhook handling via queue, idempotency key strategy)
Phase 3: PM Agent shards PRD into docs/prd/epic-1.md, epic-2.md
Phase 4: SM Agent generates docs/stories/2.1.stripe-integration.story.md
         (contains: Stripe version, webhook architecture, error handling
          strategy, acceptance criteria with test cases, security
          requirements for PCI scope)
```
...
### Brownfield projects break the model

BMAD assumes a linear flow: Brief, PRD, Architecture, Stories. Bug investigation, feature additions to existing systems, and iterative discovery workflows do not fit this sequence cleanly. The Brief, PRD, and Architecture are system-wide singletons, making multi-feature parallel work difficult. The user becomes "the coordination engine," manually relaying information between agents that have no shared context mechanism.
