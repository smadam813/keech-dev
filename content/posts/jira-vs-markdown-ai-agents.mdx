...
This site is the proof point. The `.planning/` directory contains 20 completed plans across 8 phases, executed in roughly 1 hour 29 minutes total (about 5 minutes per plan on average). Each plan is a markdown file. Each plan is version-controlled. Each plan was readable by the AI agent that executed it.
...
GitHub Issues works at roughly 10,000 issues but barely. Issues are flat, lack dependency management, and cannot compute critical paths. [TrackDown](https://github.com/nicholasgasior/trackdown), one of the git-based alternatives, explicitly targets only "small, distributed teams." When I think about where markdown-based planning would struggle most in my own work, it is in large monorepos where multiple teams need unified views and portfolio-level reporting. Cross-repo aggregation does not exist in markdown.

### The AI reliability gap

I cited the [METR study](https://metr.org/blog/2025-07-10-early-2025-ai-experienced-os-dev-study/) in my [previous post](/blog/humans-writing-code-is-over), and it is relevant here too. In a randomized controlled trial with 16 experienced developers on 246 real tasks, **developers using AI were 19% slower**, yet believed they were 20% faster. [GitHub Copilot's coding agent](https://github.blog/changelog/2025-05-19-github-copilot-coding-agent-in-public-preview/) achieves only a 40-60% success rate on well-scoped issues. Claude Code performs significantly better in my experience, but it still requires a developer who can review output critically.
