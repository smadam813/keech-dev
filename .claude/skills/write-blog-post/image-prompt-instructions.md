...
Read `src/app/globals.css` and extract the `@theme` color tokens to use as your palette. Core tokens: deep petrol background (`--color-bg: #122a32`), ivory ink (`--color-ink: #e8ecec`), mint-teal accent (`--color-accent: #8dcbbc`), runic-gold accent (`--color-accent-gold: #e0bc79`), rose accent (`--color-accent-warm: #e4a4ac`). Brutalist treatment: 2px borders with hard-offset shadows (`6px 6px 0 0 rgba(0,0,0,0.55)`), flat colors, geometric sans-serif typography, no gradients or soft shadows. Optional motif: Elder Futhark runes (Othala, Ansuz, Kenaz, Mannaz) used as ambient glyph elements — refer to `src/components/runes/rune-config.ts` if you want to incorporate them.

**Psychedelic Cosmic:**
Void black background (#000000). Bold flat colors: rose (#e4a4ac), electric blue (#3B6FC2), gold/amber (#e0bc79), neon green (#4ADB5E), crimson (#CC3030), lavender (#9878C0), nebula magenta (#6B2848). Strong graphic outlines, no gradients or soft shadows. Cosmic/psychedelic illustration style inspired by Parachute Ending animation stills.
